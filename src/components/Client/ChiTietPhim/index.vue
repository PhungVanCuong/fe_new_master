        ...
        <div class="row g-4">
            ...
            <div class="col-md-4">
                ...
                <div class="d-grid gap-3">
                    <button v-if="chi_tiet_phim.tinh_trang == 2" class="btn btn-warning py-3 fw-bold"
                        data-bs-toggle="modal" data-bs-target="#buyTicketModal">
                        <i class="fa-solid fa-ticket"></i> Mua Vé
                    </button>

                    <button v-else-if="chi_tiet_phim.tinh_trang == 1" class="btn btn-primary py-3 fw-bold">
                        <i class="fa-solid fa-bell"></i> Nhắc Lịch Chiếu
                    </button>
                </div>
            </div>
            ...
            <div class="col-md-8">
                <div class="mb-4">
                    <h1 class="fw-bold text-white mb-3">{{ chi_tiet_phim.ten_phim || 'Sample Movie' }}</h1>
                    <div class="d-flex flex-wrap gap-2 mb-3">
                        <span v-for="(value, index) in chi_tiet_phim.the_loai?.split(',') || []" :key="index"
                            class="badge rounded-pill text-white bg-light-info p-2 text-uppercase px-3">
                            {{ value || 'N/A' }}
                        </span>
                    </div>
                </div>
Chọn ngày xem
                <div class="row g-3">
                    ...
                    <div class="col-lg-6">
                        <h4 class="fw-bold text-white mb-3 border-bottom pb-2">Thông tin chi tiết</h4>
                    ...
                    <!-- Cast and Producer -->
                    <div class="col-lg-6">
                        <h4 class="fw-bold text-white mb-3 border-bottom pb-2">Diễn viên chính</h4>
                        <div class="card shadow-sm mb-4">
                            <div class="card-body">
                                <p class="mb-0">{{ chi_tiet_phim.dien_vien || 'N/A' }}</p>
                            </div>
                        </div>
                        <h4 class="fw-bold text-white mb-3  border-bottom pb-2">Nhà sản xuất</h4>
                        <div class="card shadow-sm">
                            <div class="card-body">
                ...
                <!-- Synopsis -->
                <div class="mt-4">
                    <h4 class="fw-bold text-white mb-3 border-bottom pb-2">Nội dung phim</h4>
                    <div class="card shadow-sm">
                        <div class="card-body">
            ...
            <div class="col-lg-4">
                <div class="bg-white rounded shadow-sm p-4 h-100">
                        ...
                        <div class="col-12">
                            <div class="d-flex align-items-center gap-3">
                                <div class="text-center">
                                    <h3 class="fw-bold text-success mb-0">{{ chi_tiet_phim.diem_trung_binh || 0 }}</h3>
                                    <p class="text-muted small mb-0">/5 (Tr0ondCinema)</p>
                                </div>
                                <div class="flex-grow-1">
                                    <div class="progress" style="height: 8px;">
                                        <div class="progress-bar bg-success" role="progressbar"
                                            :style="{ width: (chi_tiet_phim.diem_trung_binh / 5 * 100) + '%' }"
                                            :aria-valuenow="chi_tiet_phim.diem_trung_binh" aria-valuemin="0"
                                            aria-valuemax="5">
                                        </div>
                                    </div>
                                    <p class="text-muted small mt-1">Dựa trên {{ chi_tiet_phim.tong_danh_gia || 0 }}
                                        đánh giá</p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="mt-4">
                        <div class="row g-3">
                            <div class="col-auto">
                                <select v-model="danh_gia.so_sao" class="form-select">
                            ...
                            <div class="col">
                                <textarea v-model="danh_gia.noi_dung" class="form-control" rows="2"
                                    placeholder="Viết đánh giá của bạn..."></textarea>
                            </div>
                        </div>
                        <div class="mt-3 text-end">
                            <button @click="guiDanhGiaPhim()" class="btn btn-success px-4 py-2 fw-semibold">Gửi đánh
                                giá</button>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="row row-cols-2 row-cols-md-3 row-cols-lg-4 g-3">
                            <div class="col" v-for="(value, index) in suatChieuTheoNgay" :key="index">
                                <button class="btn w-100 py-2"
                                    :class="[
                                        kiemTraQuaGio(value.ngay_chieu, value.thoi_gian_bat_dau) 
                                            ? 'btn-secondary opacity-50' 
                                            : (selectedSuatChieuId === value.id ? 'btn-primary' : 'btn-outline-primary')
                                    ]"
                                    :disabled="kiemTraQuaGio(value.ngay_chieu, value.thoi_gian_bat_dau)"
...
export default {
    ...
    methods: {
        ...
        guiDanhGiaPhim() {
            const token = localStorage.getItem('key_client');
            if (!token) {
                this.$toast.error("Vui lòng đăng nhập để thực hiện đánh giá!");
                this.$router.push('/client/dang-nhap'); //
                return;
            }

            axios.post(apiUrl('client/chi-tiet-phim/danh-gia'), this.danh_gia, {
                headers: { Authorization: "Bearer " + token }
            })
                .then((res) => {
                    if (res.data.status) {
                        this.$toast.success(res.data.message);
                        this.danh_gia.noi_dung = ""; // Xóa trắng nội dung sau khi gửi
                        this.loadChiTietPhim(); // Gọi lại hàm này để cập nhật thanh màu xanh và điểm số mới
                    } else {
                        this.$toast.error(res.data.message);
                    }
                })
        ...
        kiemTraQuaGio(ngay, gio) {
            if (!ngay || !gio) return false;
            
            // Lấy thời gian hiện tại
            const thoiGianHienTai = new Date();
            
            // Ghép ngày và giờ của suất chiếu thành 1 chuỗi chuẩn ISO (VD: 2024-05-20T14:30:00)
            const thoiGianSuatChieu = new Date(`${ngay}T${gio}`);
            
            // Nếu thời gian suất chiếu nhỏ hơn thời gian hiện tại => Đã qua giờ
            return thoiGianSuatChieu < thoiGianHienTai;
        }
    },
}
</script>
