            ...
            <div class="card radius-10 border-top border-0 border-3 border-info">
                ...
                <div class="card-body table-responsive">
                    <div class="input-group mb-3">
                        <input v-model="search_key" type="text" class="form-control" placeholder="Tìm kiếm khách hàng (Tên, Email, SĐT...)">
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered table-hover">
                            ...
                            <tbody>
                                <!-- Đổi list_khach_hang thành filteredKhachHang. -->
                                <tr class="text-nowrap" v-for="(item, index) in filteredKhachHang" :key="index">
...
export default {
    ...
    computed: {
        filteredKhachHang() {
            // Nếu chưa gõ chữ gì thì hiện toàn bộ danh sách
            if (!this.search_key) {
                return this.list_khach_hang;
            }
            
            // Chuyển từ khóa về chữ thường để so sánh cho chính xác
            const keyword = this.search_key.toLowerCase().trim();
            
            // Lọc theo Tên, Email hoặc Số điện thoại
            return this.list_khach_hang.filter(item => {
                return (item.ho_va_ten && item.ho_va_ten.toLowerCase().includes(keyword)) ||
                       (item.email && item.email.toLowerCase().includes(keyword)) ||
                       (item.so_dien_thoai && item.so_dien_thoai.toLowerCase().includes(keyword));
            });
        }
    },
    methods: {
        loadDataKhachHang() {
            axios
                .get(apiUrl('admin/khach-hang/get-data'), {
                    headers: {
        ...
        addKhachHang() {
            axios
                .post(apiUrl('admin/khach-hang/add-data'), this.create_khach_hang, {
                    headers: {
                        Authorization: "Bearer " + localStorage.getItem('key_admin')
                    }
                })
        ...
        updateKhachHang() {
            axios
                .post(apiUrl('admin/khach-hang/update'), this.update_khach_hang, {
                    headers: {
                        Authorization: "Bearer " + localStorage.getItem('key_admin')
                    }
                })
        ...
        deleteKhachHang() {
            axios
                .post(apiUrl('admin/khach-hang/delete'), this.delete_khach_hang, {
                    headers: {
                        Authorization: "Bearer " + localStorage.getItem('key_admin')
                    }
                })
